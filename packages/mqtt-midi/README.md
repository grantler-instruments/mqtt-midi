...
```bash
# From GitHub (monorepo subpath; runs prepare → builds on install)
npm install "github:grantler-instruments/mqtt-midi#path:packages/mqtt-midi" mqtt

# Local monorepo (from repo root: npm install, then depend on workspace)
npm install ../mqtt-midi/packages/mqtt-midi mqtt
```

**Node MIDI bridge** (hardware / virtual ports): see [`@grantler-instruments/mqtt-midi-bridge`](../mqtt-midi-bridge).
...
`in` and `out` are two topic buses on the same `prefix`. They are named from the **remote** side (the device, bridge, or instrument you talk to)—not “into your app.”

| Direction | Meaning | Typical sender | Typical listener |
|-----------|---------|----------------|------------------|
| **`in`** | Commands **to** the remote | Web UI, sequencer | Device / bridge |
| **`out`** | Events **from** the remote | Device / bridge | Web UI, logger |

Examples with `prefix: "remote"`:

```text
remote/in/noteon/1/60    → “play this note on the remote”
remote/out/noteon/1/60   → “the remote played this note”
```

**Library defaults (web controller):**

- `send*` → `{prefix}/in/...`
- `on()` / `channel().on()` → `{prefix}/out/...`

So you **send commands on `in`** and **listen for what the remote did on `out`**. Your own `sendNoteOn` does not trigger your `on("noteOn")` unless something else publishes on `out`.

A **device bridge** does the opposite: subscribe to `in`, publish on `out`.

Override direction on send: `mqttMidi.sendNoteOn(1, 60, 100, "out")`. For listeners, use `defaultDirection: "in"` in the constructor or `{ direction: "in" }` on `channel().on(...)`.
